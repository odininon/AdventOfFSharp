...
module WeatherUnderground =
    ...
    let getCurrentConditionsAsync location =
        task {
            ...
            let callString =
                sprintf "http://api.wunderground.com/api/%s/conditions/q/%s/%s.json" apiKey location.State location.City
            ...
            return jobject.GetValue "weather"
                   |> string
...
module Monitor =
    ...
    let Run([<OrchestrationTrigger>] monitorContext : DurableOrchestrationContext, log : ILogger) =
        task {
            let logOnce s =
                if (not monitorContext.IsReplaying) then log.LogInformation s

            let input = monitorContext.GetInput<MonitorRequest>()
            if (obj.ReferenceEquals(input, null)) then failwith "An input object is required."
            else if (obj.ReferenceEquals(input.Location, null)) then failwith "A location input is required."
            else if (String.IsNullOrEmpty(input.Phone)) then failwith "A phone number input is required."
            let locationText =
            ...
            let rec checkWeather() =
                task {
                    if (monitorContext.CurrentUtcDateTime < endTime) then
                        // Check the weather
                        logOnce
                            (sprintf "Checking current weather conditions for %s at %A." locationText
    ...
    let GetIsClear([<ActivityTrigger>] location : Location) = task { let! currentConditions = WeatherUnderground.getCurrentConditionsAsync
                                                                                                  location
                                                                     return currentConditions = WeatherCondition.Clear }

    [<FunctionName("E3_SendGoodWeatherAlert")>]
    let SendGoodWeatherAlert([<ActivityTrigger>] phoneNumber : string,
                             [<TwilioSms(AccountSidSetting = "TwilioAccountSid", AuthTokenSetting = "TwilioAuthToken",
                                         From = "%TwilioPhoneNumber%")>] messageCollector : ICollector<CreateMessageOptions>) =
        task {
